...
_SALLA_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
if _SALLA_ROOT not in sys.path:
    sys.path.insert(0, _SALLA_ROOT)
...
router = APIRouter(prefix="/integrations/salla", tags=["salla"])
...
async def _verify_salla_signature(request: Request) -> bytes:
...
async def products_webhook(request: Request):
    await _verify_salla_signature(request)
