...
Repository Alembic graph has parallel heads ``0088`` (A1-Validate) and ``0089``
(conversation A1-subject bindings), both branching from ``0087``.
...
Integration and local bootstrap pin to ``0089`` explicitly so capability remains
...
**two rows** (``0088`` + ``0089``). Bootstrap upgrade to ``0089`` is idempotent and
must not downgrade or re-run ``0088``.
"""
from __future__ import annotations

REPOSITORY_ALEMBIC_HEADS = frozenset({"0088", "0089"})
INTEGRATION_BOOTSTRAP_TARGET = "0089"
