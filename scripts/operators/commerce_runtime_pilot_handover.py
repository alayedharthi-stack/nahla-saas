...
(revision ``0110``), which every replica reads and this job writes, and each
step is a command:
...
COMMANDS = ("status", "drain", "dispose", "retire", "settle", "reopen")
...
def cmd_retire(db: Any, tenants: Sequence[int], args: Any) -> int:
    ...
    if not name or not by or not reason:
        ...
        return EXIT_USAGE
    retired = [tenant_id for tenant_id in tenants
...
_HANDLERS = {"status": cmd_status, "drain": cmd_drain, "dispose": cmd_dispose,
             "retire": cmd_retire, "settle": cmd_settle, "reopen": cmd_reopen}
