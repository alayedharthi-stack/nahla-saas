#!/usr/bin/env python3
"""Guarded staging operator: Alembic 0016 → 0024 only.
...
_VERIFY_SALLA_SCRIPT = _REPO_ROOT / "scripts" / "verify_salla_no_duplicates.py"
...
_WABA_DUPLICATE_GROUPS_SQL = text(
    """
    SELECT count(*)::int
    FROM (
...
_ORDER_DUPLICATE_GROUPS_SQL = text(
    """
    SELECT count(*)::int
    FROM (
...
_ZOMBIE_AUTOMATION_COUNT_SQL = text(
    """
    SELECT count(*)::int
    FROM smart_automations
    WHERE automation_type IN ('cart_recovery', 'reorder_reminder', 'welcome_message')
    """
)
...
def validate_start_revision(conn: Connection) -> GateFailure | None:
    ...
    if current != BASE_REVISION:
        return GateFailure("wrong_revision", "start_revision_not_0016")
    return None


def collect_destructive_preflight_counts(conn: Connection) -> dict[str, int]:
    return {
        "waba_duplicate_groups": int(conn.execute(_WABA_DUPLICATE_GROUPS_SQL).scalar_one()),
        "order_duplicate_groups": int(conn.execute(_ORDER_DUPLICATE_GROUPS_SQL).scalar_one()),
        "zombie_automation_rows": int(conn.execute(_ZOMBIE_AUTOMATION_COUNT_SQL).scalar_one()),
    }


def run_salla_preflight(
    *,
    python_executable: str | None = None,
    runner: Callable[..., subprocess.CompletedProcess[str]] | None = None,
) -> str:
    """Run verify_salla_no_duplicates.py; return safe outcome token only."""
    exe = python_executable or sys.executable
    cmd = [exe, str(_VERIFY_SALLA_SCRIPT)]
...
def build_alembic_upgrade_command(python_executable: str | None = None) -> list[str]:
    """Construct the only permitted Alembic invocation (fixed target 0024)."""
    exe = python_executable or sys.executable
...
def validate_post_success_revision(conn: Connection) -> GateFailure | None:
    ...
    if current != TARGET_REVISION:
        return GateFailure("post_validation_failed", "revision_not_0024")
    return None
...
def run_preflight(
    ...
    *,
    ...
    run_salla_check: bool = True,
    salla_runner: Callable[..., subprocess.CompletedProcess[str]] | None = None,
    ...
    salla_outcome: str | None = None
    if run_salla_check:
        salla_outcome = run_salla_preflight(runner=salla_runner)
        if salla_outcome != "pass":
            stage = {
                "fail": "duplicate_salla_integrations",
                "timeout": "salla_preflight",
                "spawn_failed": "salla_preflight",
                "unexpected_error": "salla_preflight",
            }[salla_outcome]
            error_class = {
                "fail": "salla_preflight_failed",
                "timeout": "salla_preflight_timeout",
                "spawn_failed": "salla_preflight_spawn_failed",
                "unexpected_error": "salla_preflight_unexpected_error",
            }[salla_outcome]
            return None, GateFailure(error_class, stage)
    ...
    manifest = build_manifest(
        ...
        salla_preflight_outcome=salla_outcome,
    )
...
def run_controlled_migration(
    ...
    *,
    ...
    manifest = build_manifest(
        ...
        salla_preflight_outcome=preflight_manifest.get("salla_preflight_outcome"),
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Guarded staging operator: Alembic 0016 → 0024 only.",
        exit_on_error=False,
    )
