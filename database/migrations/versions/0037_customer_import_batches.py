...
the table or index while ``alembic_version`` is still at 0036.
"""
...
def upgrade() -> None:
    ...
    if not has_table(bind, "customer_import_batches"):
        op.create_table(
    ...
    if has_table(bind, "customer_import_batches") and not has_index(
        bind, "customer_import_batches", "ix_customer_import_batches_tenant_status",
    ):
        op.create_index(
