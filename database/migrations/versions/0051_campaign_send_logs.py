...
Idempotency (F16)
─────────────────
Wrapped in inspector guards. If ``campaign_send_logs`` already exists
in the deployed schema (e.g. created via legacy
``Base.metadata.create_all()`` before Alembic was wired in), the
table creation and every index creation are skipped — behaviour on a
clean DB is unchanged.
"""
...
def upgrade() -> None:
    ...
    if not _has_index(bind, "campaign_send_logs", "uq_campaign_send_log_campaign_phone"):
        op.create_index(
    ...
    if not _has_index(bind, "campaign_send_logs", "ix_campaign_send_log_tenant_phone_status_sent"):
        op.create_index(
            "ix_campaign_send_log_tenant_phone_status_sent",
            "campaign_send_logs",
    ...
    if not _has_index(bind, "campaign_send_logs", "ix_campaign_send_log_campaign_status"):
        op.create_index(
