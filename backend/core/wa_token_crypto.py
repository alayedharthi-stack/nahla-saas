...
def _fernet():
    ...
    key = (os.getenv("WA_TOKEN_ENC_KEY") or os.getenv("TOTP_ENC_KEY") or "").strip()
    if key:
        try:
            return Fernet(key.encode("utf-8"))
        except Exception as exc:  # noqa: BLE001
            raise RuntimeError(
                f"WA_TOKEN_ENC_KEY is set but invalid (must be a Fernet key). Error: {exc}"
            ) from exc
    ...
    logger.warning(
        "[wa_token_crypto] WA_TOKEN_ENC_KEY not set — using DEV fallback. "
