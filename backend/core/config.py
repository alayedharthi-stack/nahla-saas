...
# Dedicated Embedded Signup configuration for WhatsApp Business App coexistence
# (featureType=whatsapp_business_app_onboarding). Fail closed when unset — no
# fallback to Cloud API / META_WA_CONFIG_ID.
META_COEXISTENCE_EMBEDDED_SIGNUP_CONFIG_ID = (
    os.environ.get("META_COEXISTENCE_EMBEDDED_SIGNUP_CONFIG_ID", "").strip()
)


def is_coexistence_embedded_signup_available() -> bool:
    return bool(META_COEXISTENCE_EMBEDDED_SIGNUP_CONFIG_ID)


def meta_coexistence_embedded_signup_config_id() -> str:
    return META_COEXISTENCE_EMBEDDED_SIGNUP_CONFIG_ID
...
ORDER_CONTEXT_SHIPPING_CONFIRM_ENABLED = _bool_env(
    "ORDER_CONTEXT_SHIPPING_CONFIRM_ENABLED",
    "false",
)
