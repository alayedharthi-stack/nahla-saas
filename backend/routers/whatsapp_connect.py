...
from services.whatsapp_platform.provider_utils import (
    ...
    provider_is_supported as _provider_is_supported,
    raw_provider as _raw_provider,
...
_UNSUPPORTED_PROVIDER_MESSAGE = (
    "هذه القناة مرتبطة بمزوّد واتساب لم يعد مدعومًا. "
    "الرجاء إعادة الربط عبر Meta WhatsApp Cloud API."
)


def _unsupported_provider_payload(conn: Optional[WhatsAppConnection]) -> Dict[str, Any]:
    """What a connection row from a retired provider reports.

    Never "connected", and never Meta's shape: the credential on this row is
    one nothing can use, so reporting it as a working channel would put a dead
    number on the merchant's dashboard.
    """
    return {
        "status": "unsupported_provider",
        "connected": False,
        "provider": WHATSAPP_PROVIDER_UNSUPPORTED,
        "stored_provider": _raw_provider(conn),
        "sending_enabled": False,
        "message": _UNSUPPORTED_PROVIDER_MESSAGE,
    }
...
def _has_recent_webhook_traffic(
    ...
    *,
    ...
    """True iff this connection received a Meta webhook recently.
...
async def live_verify_connection(request: Request, db: Session = Depends(get_db)):
    ...
    -------
    {
      ...
      checks: [
        ...
        {name: 'provider_reachable', ok: bool, status_code: int|null,
         provider: 'meta'|'unsupported', detail: str|null},
      ],
    ...
    if has_token and (phone_id or waba_id):
        ...
        except UnsupportedWhatsAppProvider as unsupported:
            # A connection left over from a retired provider. It is not
            # reachable, and it is not Meta's to answer for.
            provider_detail = str(unsupported)
            provider_reachable = False
...
async def reconnect(request: Request, db: Session = Depends(get_db)):
    ...
    if not _provider_is_supported(conn):
        # Nothing to reconnect: the provider this row names is retired. The
        # merchant connects through Meta, which is a different flow entirely,
        # and this row is left exactly as it is for the record.
        return _unsupported_provider_payload(conn)
...
async def refresh_meta_tier(
    ...
    verify what's actually being returned.
    """
    from services.whatsapp_platform.service import fetch_meta_phone_tier  # noqa: PLC0415
...
async def connection_health(request: Request, db: Session = Depends(get_db)):
    ...
    if not _provider_is_supported(conn):
        return {
            "healthy": False,
            "phone_number": conn.phone_number,
            "checks": {"has_connection": False, "token_present": False, "token_valid": False,
                       "webhook_verified": False, "sending_enabled": False},
            "last_verified": conn.last_verified_at.isoformat() if conn.last_verified_at else None,
            "last_error": conn.last_error,
            **_unsupported_provider_payload(conn),
        }
...
async def whatsapp_status(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        tenant_id = resolve_tenant_id(request)
        conn = db.query(WhatsAppConnection).filter_by(tenant_id=tenant_id).first()
        if conn and not _provider_is_supported(conn):
            return _unsupported_provider_payload(conn)
