...
──────────────────────
Internal-only diagnostics surface for support/recovery work.

Exposes a handful of admin-gated diagnostic endpoints (Salla cleanup
preview, WhatsApp send-template probe, media env, etc.). Each handler
calls ``_require_enabled`` which honours ``ENABLE_ADMIN_DEBUG`` +
optional ``ADMIN_DEBUG_SECRET``.

Phase 1A change
───────────────
The previous ``GET/POST /admin/debug/reset-admin`` endpoint was removed
(May 2026) — its query-string default password (``12345678``) was an
unauthenticated path to platform-admin if a misconfigured deploy ever
flipped ``ENABLE_ADMIN_DEBUG=true``. Use the offline script instead:

    railway run python scripts/reset_admin_password.py \\
        --email admin@nahlah.ai --password "$(openssl rand -base64 24)"

The script writes the bcrypt hash directly into the ``users`` table via
``DATABASE_URL`` and never opens an HTTP surface.

⚠️  SECURITY
-----------
``ENABLE_ADMIN_DEBUG`` MUST stay ``false`` in production. Flip it on
only for the duration of a single recovery action and turn it back off
immediately afterwards.

Optionally protect with a shared secret (``ADMIN_DEBUG_SECRET``) — when
set, every endpoint here requires ``?secret=<value>`` to match.
"""
...
# NOTE: ``GET/POST /admin/debug/reset-admin`` was removed in May 2026
# (Phase 1A security hardening). The query-string default password
# ``12345678`` made it an unauthenticated path to platform-admin if a
# misconfigured deploy ever flipped ``ENABLE_ADMIN_DEBUG=true``. The
# replacement is an offline CLI script that runs from a Railway shell
# and writes the bcrypt hash directly to the DB without opening an
# HTTP surface:
#
#     railway run python scripts/reset_admin_password.py \
#         --email admin@nahlah.ai --password "$(openssl rand -base64 24)"
#
# Any 410 GET/POST hitting the old path lands on the explicit ``Gone``
# handler below so external callers get a clear error message instead
# of a silent 404 (which a load balancer might mistake for "wait, I'll
# retry").


@router.api_route("/reset-admin", methods=["GET", "POST"], include_in_schema=False)
def reset_admin_gone(_: Optional[str] = Query(None, alias="secret")):
    """410 Gone — endpoint removed for security (Phase 1A)."""
    raise HTTPException(
        status_code=410,
        detail=(
            "Endpoint removed for security. Use the CLI script instead: "
            "`railway run python scripts/reset_admin_password.py --email <e> --password <p>`."
        ),
    )
...
# through the live Meta connection of any tenant WITHOUT going
...
# reproduction we can hand to Meta support.
#
...
def _extract_provider_message_id(resp_data: Optional[Dict[str, Any]]) -> Optional[str]:
    """Pull ``messages[0].id`` out of a Meta response.
...
async def admin_debug_send_template(
    ...
    Returns the raw provider response (Meta) so support
    ...
    Success shape:
      {
        "ok": bool,
        "http_status": 200,
        "provider": "meta_cloud" | "unsupported",
    ...
    from services.whatsapp_platform.provider_utils import provider_is_supported  # noqa: PLC0415
    ...
    provider_label = "meta_cloud" if provider_is_supported(wa_conn) else provider_kind
    ...
    * The endpoint never raises 5xx on observability failures; if
    ...
    if classification_counts.get("missing_wamid", 0) > 0:
        issues.append(
            ...
            "الإرسال فشل صامتاً قبل F18. تحقّق من صلاحية رمز Meta و "
