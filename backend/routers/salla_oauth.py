...
# Short-lived launch token used by /app/salla/launch completion handoff.
_LAUNCH_TOKEN_TTL_SECONDS = 120
...
def _issue_launch_token(
    *,
    ...
    exp = now + timedelta(seconds=_LAUNCH_TOKEN_TTL_SECONDS)
    launch_payload = {
        "sub":          email,
        "role":         role,
        "tenant_id":    tenant_id,
    ...
    return _jose_jwt.encode(launch_payload, JWT_SECRET, algorithm=JWT_ALGORITHM)


def _build_launch_handoff_url(launch_jwt: str, next_path: str | None = None) -> str:
    safe_next = _sanitize_internal_next_path(
        next_path or _OAUTH_EMBEDDED_RECONCILE_NEXT,
        default=_OAUTH_EMBEDDED_RECONCILE_NEXT,
    )
    params = urllib.parse.urlencode({"token": launch_jwt, "next": safe_next})
...
async def salla_token_login(request: Request, db: Session = Depends(get_db)):
    ...
    # ══════════════════════════════════════════════════════════════
    #
    # All identity resolution lives in core.merchant_provisioning.
    # That module guarantees:
    #   * salla_store_id is the AUTHORITATIVE lookup key
    #   * Email fallback only fires when introspect gave us no store_id
    #   * Returning stores never produce a duplicate User row
    #   * If a User row is auto-created, a single-use set-password
    #     token is issued and returned in ``provision.set_password_token``
    #
...
async def launch_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        launch_jwt = _issue_launch_token(
            email=email,
            role=role,
            tenant_id=tenant_id,
            user_id=user_id,
    ...
    launch_url = _build_launch_handoff_url(launch_jwt, safe_next)
...
async def resolve_launch(request: Request, db: Session = Depends(get_db)):
    ...
    launch_jti = str(decoded.get("jti") or "").strip()
    launch_exp = decoded.get("exp")
    if not launch_jti:
        logger.warning("[ResolveLaunch] launch token missing jti")
        raise HTTPException(
            status_code=401,
            detail="تعذر تسجيل الدخول من سلة، حاول فتح التطبيق مرة أخرى.",
        )
    from core.token_revocation import try_consume_one_time_jti  # noqa: PLC0415
    if not try_consume_one_time_jti(launch_jti, launch_exp, namespace="launch"):
        logger.warning("[ResolveLaunch] launch token replay rejected | jti=%s", launch_jti[:8])
        raise HTTPException(
            status_code=401,
            detail="تعذر تسجيل الدخول من سلة، حاول فتح التطبيق مرة أخرى.",
        )
...
async def salla_api_oauth_callback(
    ...
    if embedded_reconcile:
        ...
        if not merchant_user:
            logger.error(
                "[Salla API OAuth] embedded reconcile missing merchant user | tenant=%s store_id=%s",
                tenant_id, salla_store_id,
            )
            return RedirectResponse(
        ...
        try:
            launch_jwt = _issue_launch_token(
                ...
                role=str(merchant_user.role or "merchant"),
                tenant_id=tenant_id,
            ...
            logger.exception(
                "[Salla API OAuth] embedded reconcile launch token failed | tenant=%s store_id=%s err=%s",
                tenant_id, salla_store_id, exc,
            )
            return RedirectResponse(
                url=f"{_DASHBOARD_ORIGIN}/app/salla?{urllib.parse.urlencode({'salla_oauth': 'error', 'reason': 'launch_token_failed'})}",
                status_code=302,
            )
        success_url = _build_launch_handoff_url(launch_jwt, _OAUTH_EMBEDDED_RECONCILE_NEXT)
        logger.info(
...
async def salla_oauth_callback(
    ...
    if is_new_merchant:
        ...
        try:
            ...
            email_is_derived_legacy = salla_email.endswith("@salla-merchant.nahlah.ai")
            salla_email = salla_email.strip().lower()
            logger.info("[Salla OAuth] Merchant email resolved: %s", salla_email)

            # All identity resolution lives in the shared provisioning
            # helper. See core.merchant_provisioning for the lookup
            # priority and the password-decoupling guarantee.
            try:
                provision_legacy = get_or_create_merchant_user(
                    db,
                    provider          = "salla",
                    external_store_id = str(salla_store_id or ""),
            ...
            if provision_legacy.linked_existing:
                audit("salla_oauth_linked_existing", sub=salla_email,
                      tenant_id=tenant_id, store_id=salla_store_id)
                logger.info(
                    "[Salla OAuth] linked existing | tenant=%s user_id=%s store_id=%s",
                    tenant_id, provision_legacy.user_id, salla_store_id,
                )
            elif provision_legacy.filled_gap:
                audit("salla_oauth_user_filled_gap", sub=salla_email,
                      tenant_id=tenant_id, store_id=salla_store_id)
            elif provision_legacy.created_user:
                audit("salla_oauth_merchant_auto_created", sub=salla_email,
                      tenant_id=tenant_id, store_id=salla_store_id)
                logger.info(
                    "[Salla OAuth] merchant auto-created | tenant=%s user_id=%s store_id=%s",
                    tenant_id, provision_legacy.user_id, salla_store_id,
                )

            # Issue the embedded JWT used by SallaCallback.tsx — no
            # change to behaviour, just reuses the resolved identity.
            auto_jwt = create_token(
                email     = salla_email,
                role      = provision_legacy.role,
                tenant_id = tenant_id,
                user_id   = provision_legacy.user_id,
            )

            # Welcome + set-password email — only on a brand-new auto-create
            # AND only when the email is real (not a derived placeholder).
            if (
                provision_legacy.set_password_token
                and not email_is_derived_legacy
                and not salla_email.endswith("@salla-merchant.nahlah.ai")
            ):
                asyncio.ensure_future(_send_welcome_set_password_email(
                    email             = salla_email,
                    store_name        = store_name or "متجرك",
                    set_password_url  = f"{_DASHBOARD_ORIGIN}/set-password?token={provision_legacy.set_password_token}",
                    dashboard_url     = _DASHBOARD_ORIGIN,
                ))
                logger.info(
                    "[Salla OAuth] welcome+set-password email queued | user_id=%s tenant=%s",
                    provision_legacy.user_id, tenant_id,
                )
