...
def recover_tenant(db: Any, *, tenant_id: int, limit: int = 50,
    ...
    admits = handover.barrier_admits_new_work(db, tenant_id=int(tenant_id))
    ...
    for record in entries:
        ...
        if not admits:
            # Selected before a drain, refused at admission: the entry stays
            # exactly as it is, and the run says so rather than replaying into
            # a barrier that would refuse it anyway.
            done(SKIPPED_BARRIER_CLOSED)
            continue
...
__all__ = ["EntryOutcome", "FAILED", "RESOLVED_ALREADY_FINISHED", "REPLAYED",
           "RecoveryReport", "SKIPPED_BARRIER_CLOSED", "SKIPPED_IN_FLIGHT",
           "SKIPPED_NOT_REPLAYABLE", "SKIPPED_UNKNOWN_DELIVERY", "recover",
           "recover_tenant"]
