...
def should_slim_general_brain_state(state: BrainReplyState) -> Tuple[bool, str]:
    ...
    if isinstance(facts, dict) and facts.get("customer_request_coupon_facts"):
        return False, "customer_request_coupon_facts"
    ...
    if (
        ...
        and intent not in _slim_ok_intents
    ):
        return False, "catalog_reasoning_evidence"

    if (
        isinstance(facts, dict)
        and facts.get("customer_order_evidence")
        and intent not in _slim_ok_intents
    ):
        return False, "customer_order_evidence"
