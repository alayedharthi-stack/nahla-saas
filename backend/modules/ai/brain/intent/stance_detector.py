...
_OBJECTION_PATTERNS = _compile(
    ...
    r"(?:سعر(?:ه|ها|هم)?|السعر)\s*(?:غالي|غالية|عالي|عالية|مرتفع|كثير|كثيرة|زايد)",
    r"غالي\s*(?:يقول|قال|ب)\s*\d{2,4}",
    r"عند\s+(?:واحد|فلان|محل|منافس|غير(?:كم|ك)?)",
    r"(?:لقيت(?:ه)?|اشتري(?:ت)?)\s+(?:من\s+)?(?:عند\s+)?",
    r"(?:أ?رخص|أغلى)\s+من(?:كم|ك)?",
    r"(?:إ?ذا|لو)\s+.{0,40}(?:نزل(?:ت)?|ب\s*\d{2,4})",
    # Trust pushback.
...
STANCE_DIRECTIVES: Dict[str, str] = {
    ...
    STANCE_DEFERRED: (
        "relational_frame=deferred — العميل يؤجّل الشراء بهدوء، وغالبًا يقصد أن "
        "منتجًا سابقًا اشتراه «لا يزال عنده ولم ينتهِ». الرد المطلوب يربط الكلام "
        "بالمنتج السابق نفسه ولا يخرج إلى عبارات عامة: "
        "(1) اعترفي بكلامه بصياغة مرتبطة بالمنتج السابق مثل «الله يبارك لك فيه / "
        "متّعك الله به / يطيب فيك / عافية على عمر» — لا «دوم بخير» ولا «تحت أمرك» "
        "ولا «أي وقت تحتاجين». "
        "(2) سطر واحد فقط يفتح الباب لاحقًا بصياغة هادئة مثل «وأي وقت تحتاجين "
        "الجديد نحن هنا» — بدون CTA إلزامي وبدون سؤال «تحبين أرشّح لك؟». "
        "ممنوع تمامًا: pitch بيعي، اقتراح منتج بديل، خصومات تلقائية، عبارات "
        "تحية عامة لا ترتبط بسياق المنتج (دوم بخير / إن شاء الله / كل عام)."
    ),
    STANCE_POLITE_CLOSE: (
    ...
    STANCE_SOCIAL_BONDING: (
        "relational_frame=social_bonding — رسالة دافئة/دينية أو مديح لتعزيز العلاقة. "
        "ردّي بمقابل واحد مختصر وطبيعي كتاجر سعودي على واتساب («الله يسعدك / "
        "تسلم / ويسعد أيامك / حياك الله / وإياك / آمين») — ممنوع تكديس أكثر "
        "من دعاء في رسالة واحدة ولا عبارات أدبية أو غير سعودية (يطري أيامك / "
        "دوم بخير / ولك بمثل ما دعيت / دوم إحساسك / دمت بود / يسعد مساك على "
        "شعورك). ثم استكملي خيط الحوار الموضوعي السابق إن وُجد — لا تُحوّليها "
        "إلى بداية محادثة جديدة ولا pitch بيعي."
    ),
    STANCE_INFO_ONLY: (
