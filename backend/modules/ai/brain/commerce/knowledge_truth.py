...
def structured_conflicts_for_kinds(
    ...
    *,
    ...
    if has_catalog and kinds.intersection({"product_usage", "product_recipe", "product_extra"}):
        conflicts.append("catalog_structured_wins")
    if has_branches and kinds.intersection({"branch", "store_location", "location"}):
        conflicts.append("branches_structured_wins")
    if has_contacts and kinds.intersection({"contact", "staff_contact", "escalation"}):
        conflicts.append("contacts_structured_wins")
    if has_promotions and kinds.intersection({"coupon", "offer", "promotion", "discount"}):
        conflicts.append("promotions_structured_wins")
    if has_payments and kinds.intersection({"payment_method", "bank_transfer"}):
