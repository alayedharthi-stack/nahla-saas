...
def _lookup_tenant_store_url(db: Any, tenant_id: int) -> str:
    ...
    Delegates to :mod:`modules.ai.brain.commerce.store_url_resolver`.
    """
    try:
        from modules.ai.brain.commerce.store_url_resolver import (  # noqa: PLC0415
            lookup_tenant_store_url as _resolve,
        )
...
def _build_store_link_reply(store_url: str) -> str:
    """Inject configured store URL — operational fact, no marketing wrapper."""
    return str(store_url or "").strip()
