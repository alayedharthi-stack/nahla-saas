"""Integration regressions for conditional-coupon compose provenance wiring."""
...
_COMPOSE_BODY = "بعد 3 طلبات مكتملة يتفعل عرض الكوبون حسب بيانات المتجر."
...
def test_pipeline_conditional_coupon_provenance_after_sanitize() -> None:
    ...
    try:
        assert result.get("customer_conditional_coupon_compose_active") is True
        assert result.get("compose_source") == "persona_llm"
        assert result.get("chosen_path") == "customer_conditional_coupon_compose"
...
def test_webhook_dedup_updates_conditional_coupon_provenance_metadata() -> None:
    ...
    def _dedup_history(*_args, **_kwargs):
        return [
            {"direction": "inbound", "body": _MIN_ORDERS_MESSAGE},
    ...
    assert saved_metadata.get("customer_conditional_coupon_compose_active") is True
    assert saved_metadata.get("compose_source") == "persona_llm"
    assert saved_metadata.get("chosen_path") == "customer_conditional_coupon_compose"
