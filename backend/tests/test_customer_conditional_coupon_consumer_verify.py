...
def test_operator_present_at_pinned_target_runtime_revision() -> None:
    proc = subprocess.run(
        [
            "git",
            "cat-file",
...
def test_operator_absent_at_prior_target_runtime_revision() -> None:
    prior = "10848a4633723fa3f4aff7801fc92fd1ac393efb"
    proc = subprocess.run(
        [
            "git",
            "cat-file",
            "-e",
            f"{prior}:scripts/operators/customer_conditional_coupon_consumer_verify.py",
        ],
        cwd=_REPO,
        capture_output=True,
        check=False,
