...
def _canary_steps() -> List[ReplayStep]:
    return [
        ...
        ReplayStep(
            ...
            "سعدية الحارثي",
            live_route_owner="brain.pipeline",
            live_outbound_snippet=_CATALOG_BROWSE_PHRASE,
        ),
        ReplayStep("picked", "انا اخترت المنتجات", live_route_owner="brain.pipeline"),
        ReplayStep("nudge", "شفيك", live_route_owner="brain.pipeline"),
        ReplayStep(
            "delivery",
            "ودوه لعنواني",
            live_route_owner="brain.pipeline",
            live_outbound_snippet=_LIVE_SHIPPING_SNIPPET,
        ),
        ReplayStep(
            "bank",
            "الراجحي",
            live_route_owner="brain.pipeline",
            live_outbound_snippet="بيانات التحويل",
        ),
    ]
...
class TestBrainReplayCanaryShippingPaymentProviderMismatch:
    def test_canary_thread_emits_route_owner_audit(self) -> None:
        ...
        audit = runner.run_thread(_canary_steps())
        ...
        assert "brain" in by_label["browse_1"].route_owner
