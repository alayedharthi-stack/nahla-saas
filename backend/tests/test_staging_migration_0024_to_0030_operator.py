"""Unit tests for guarded staging migration 0024→0030 operator."""
...
from scripts.operators import staging_migration_0024_to_0030 as op  # noqa: E402
...
def test_contract_matches_legacy_fixtures() -> None:
    assert TARGET_REVISION == FIXTURE_TARGET == "0030"
...
def test_confirmation_token_distinct_from_0016_runner() -> None:
    assert CONFIRMATION_TOKEN != "RUN_STAGING_0016_TO_0024"
    assert CONFIRMATION_ENV != "NAHLA_STAGING_MIGRATION_CONFIRM"


def test_wrong_start_revision_rejected() -> None:
    engine = _sqlite_engine_with_revision("0025")
    with engine.connect() as conn:
        failure = op.validate_start_revision(conn)
    assert failure is not None
    assert failure.error_class == "wrong_revision"
    assert failure.stage == "start_revision_not_0024"


def test_upgrade_command_fixed_target_no_head() -> None:
    cmd = op.build_alembic_upgrade_command("python")
    assert cmd == ["python", "-m", "alembic", "upgrade", "0030"]
    assert "head" not in " ".join(cmd)
    op.assert_upgrade_command_safe(cmd)


def test_upgrade_command_rejects_non_0030_target() -> None:
    with pytest.raises(ValueError, match="unsafe_command_shape"):
        op.assert_upgrade_command_safe(["python", "-m", "alembic", "upgrade", "0031"])
...
def test_manifest_contains_no_pii_keys() -> None:
    ...
    with (
        ...
        patch.object(
            ...
            return_value={
                "preexisting_product_interests_table": 0,
                "preexisting_promotions_table": 0,
                "preexisting_offer_decisions_table": 0,
                "products_stock_columns_preexisting": 0,
                "orders_dashboard_columns_preexisting": 0,
                "platform_tenant_column_preexisting": 0,
                "smart_automation_engine_backfill_rows": 0,
            },
        ),
    ):
...
def test_run_aborts_without_confirmation() -> None:
    env = _staging_env()
...
def test_run_controlled_migration_rejects_post_revision_mismatch() -> None:
    ...
    with (
        ...
        patch.object(
            ...
            return_value=op.GateFailure("post_validation_failed", "revision_not_0030"),
        ),
    ):
        manifest, failure = op.run_controlled_migration(
    ...
    assert manifest is None
    assert failure == op.GateFailure("post_validation_failed", "revision_not_0030")
