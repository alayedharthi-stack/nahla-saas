...
from routers.conversations import reply_to_conversation  # noqa: E402
...
class TestBManualReplyDoesNotControlAi:
    def test_reply_source_does_not_stamp_takeover(self) -> None:
        ...
        assert "taken_over_at" not in src
        assert 'reason="staff_takeover"' not in src
        assert "create_handoff_session" not in src
        assert "is_human_handoff = True" not in src
        compact = src.replace(" ", "")
