...
def test_resolve_slot_model_uses_canonical_default(monkeypatch):
    ...
    monkeypatch.delenv("NAHLA_MODEL_TINY", raising=False)

    from modules.ai.brain.intent.slot_extractor import _resolve_slot_model

    assert _resolve_slot_model() == "gpt-5.6-luna"
...
def test_slot_extractor_logs_safe_diagnostics_on_api_connection_error(
    ...
    monkeypatch.setenv("OPENAI_API_KEY", _SECRET_MARKER)
    monkeypatch.delenv("ANTHROPIC_SLOT_MODEL", raising=False)
    monkeypatch.delenv("NAHLA_MODEL_TINY", raising=False)
...
def test_slot_extractor_uses_resolved_model_for_cost_audit(monkeypatch):
    ...
    monkeypatch.setenv("ANTHROPIC_SLOT_MODEL", "gpt-5.6-terra")
    ...
    async def _run():
        with patch(
            "modules.ai.orchestrator.providers.openai_compatible_provider.OpenAICompatibleProvider.call",
            return_value={"reply_text": '{"intent_hint":"general"}', "model": "gpt-5.6-terra"},
        ):
            with patch(
    ...
    assert audited_models == ["gpt-5.6-terra"]
