...
|------|---------------|---------|
| Coupons | **Partial — one cleanup PR** | Hide technical sync details (`SHOW_COUPON_SYNC_TECH_DETAILS`) |
...
**Still `true`** in `dashboard/src/utils/couponSallaSyncError.tsx` (line 11).

Merchants still see technical hints (HTTP codes, field keys) on sync errors. **Must be set to `false` before sign-off.**
...
| No technical error details in UI | **Fail — flag still true** |
| Full API link required for push | Documented; merchant UI in #453 |

**Remaining coupon work:** one-line PR — `fix(coupons): hide technical sync details before Salla sign-off`.
...
|------|----------------|--------|------|-------------|
| **Coupons** | Two-way sync prod-validated; name + dates aligned | **Partial** | `SHOW_COUPON_SYNC_TECH_DETAILS=true` | `fix(coupons): hide technical sync details before Salla sign-off` |
...
1. `fix(coupons): hide technical sync details before Salla sign-off`
2. `fix(orders): preserve Salla monetary totals`
3. `fix(offers): show Salla offer sync status` (visibility first)
4. `chore(offers): audit Salla offer API surface` (if separate research PR needed)
5. `fix(products): align Salla product price and variant sync`
