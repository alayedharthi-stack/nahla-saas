...
| `commerce_runtime_agent_loop` | proof | `tests/commerce_reliability/test_commerce_runtime_agent_loop_pg.py` | agent loop PR, dormant agent loop core and its durable guarantees (reasoning with read-only fixture tools and observations feeding the next decision; revision-bound attempt debits, concurrency arbitration and re-entry restoration; enforced provider and tool waits with the deadline re-checked inside the reservation transaction; scope, eligibility and ownership boundaries; complete provider-result validation; isolation of authoritative schemas and context; closed ownership-loss outcomes; bundle-duplicate refusal and the durable crash-safe recovery allowance; a step cut off at the output limit told back so the finished answer is still delivered, while no step left still stops and any other invalid output still ends the turn; and an identifier the customer's own message carried, quoted back to say it was not found and ask about it, accepted and delivered, while the identical sentence on a turn that never mentioned it is refused and corrected) | 44 |
| `commerce_runtime_pilot` | proof | `tests/commerce_reliability/test_commerce_runtime_pilot_pg.py` | pilot integration and corrected admission, ownership, delivery and recovery boundaries with scripted provider/transport; a step's tool bundle bounded by the turn's budget rather than a smaller ceiling, a stop that names its cause in the report and the terminal, and the merchant's shareable coupons read from its own records — never a campaign-only code, never another tenant's, no write to any promotion table and every coupon read bound to this tenant's id, another customer's personal code never evidence here while this customer's own is, a code the records did not produce never reaching the customer; a multi-link, multi-image listing reaching the wire unchanged through the platform's own sanitiser while a search-dump reply is still replaced, and a percentage coupon reconciled from Salla as a money object read as a percentage rather than an amount, and the products an earlier reply in this conversation was grounded on carried into the next turn as identities it may read — bounded, tenant- and conversation-scoped, each product judged on the age of the reply that last showed it so a busy conversation never keeps an old one current, and a lapsed product reached again by an ordinary search that the following turn then carries; and the tappable selector the model may offer — composed from this turn's own observations, reserved as a rich intent and sent as a list, a product the turn never looked up refused before anything is sent and the refusal correctable, the answer still delivered as text after a proven refusal of the list but never after an accepted or an unknown send, and a tapped row resolved to a product only when this conversation still carries it, and a rejection a re-entry did not itself produce still reaching the customer while an accepted one and a second recovery are refused by the ledger; a tap verified against the rows that reply actually sent rather than a product merely named in prose, and every option reaching the customer as lines of the merchant own values whenever the channel will not carry the rows; a tap resolved against the one list it names rather than any earlier list this conversation still holds, never across conversations, and the row ids read back from the wire the send path actually wrote; and the merchant's loyalty ladder read for this customer — a rung they have not reached never leaving the records whatever the store's policy allows, the merchant's own saved first-purchase rule opening the first rung for a customer with no orders while the identical case without that block keeps it shut, and a coupon tied to no rung never withheld for want of one while one the merchant never placed on a surface the assistant reads is not read as a general offer at all | 105 |
...
| `commerce_runtime_synthetic_probe` | proof | `tests/commerce_reliability/test_commerce_runtime_synthetic_probe_pg.py` | the re-runnable synthetic probe of the runtime pilot: a disposable database created, migrated, seeded and dropped; every case through the pilot's own entry with the real read tools and the wire sanitiser under a scripted provider; no second send on replay; no commerce effect; no secret in the output; the database dropped even when a case fails | 2 |
