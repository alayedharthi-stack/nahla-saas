...
ABSENCE_MARKERS = ("لم أجد", "لم نجد", "ما لقيت", "مالقيت", "ما حصلت", "ما عندنا", "ماعندنا",
                   "غير متوفر", "لا يوجد", "ما يوجد", "مو موجود", "ليس لدينا", "ماهو موجود")
...
CATALOGUE_C: Tuple[Tuple[str, int, int, Tuple[str, ...], str, bool], ...] = (
...
SCENARIOS: Tuple[Scenario, ...] = (
    ...
    # Not clearly a product request (tenant 33, turn 68 and its neighbours).
    Scenario("amb_observed_after_social", "A", ("عيال محمد عندك",),
             history=(("inbound", "ابداع روعه"), ("outbound", "الله يسعدك 🌷"))),
    Scenario("amb_observed_fresh", "A", ("عيال محمد عندك",)),
    Scenario("amb_personal_dinner", "A", ("تبغون العشا الي البارح",)),
    Scenario("amb_addressing_owner", "A", ("خالد رد علي",)),
    Scenario("amb_social_other_store", "C", ("الجماعة وصلوا عندك؟",)),
    # Clear product requests: answered, not asked back.
    Scenario("clear_black_dress", "A", ("عندكم فستان أسود؟",)),
    Scenario("clear_blouse_colloquial", "A", ("فيه بلوزه بيضا؟",)),
    Scenario("clear_browse", "A", ("وش عندكم منتجات؟",)),
...
def measure(result: Mapping[str, Any], store: Store) -> Dict[str, Any]:
    ...
    return {
        ...
        "asks_back": ("؟" in body) or ("?" in body),
        "absence_markers": [m for m in ABSENCE_MARKERS if m in body],
    }
...
def run_scenario(ctx: Context, scenario: Scenario, rep: int) -> List[Dict[str, Any]]:
    ...
    name = {"A": "نورة عبدالله", "B": "أحمد سالم", "C": "سارة محمد"}[scenario.tenant]
...
def outcome_class(record: Mapping[str, Any]) -> str:
    """Report only: what the turn did for the customer, from structure first.

    ``products`` — it cited or showed products; ``asked_back`` — it asked the
    customer what they meant and asserted nothing; ``asserted_absence`` — it
    said the store has no such thing without asking; otherwise ``other``."""
    metrics = record.get("metrics") or {}
    refs = [r for r in str(record.get("evidence_refs") or "").split(",") if r.startswith("catalog:")]
    if refs or metrics.get("rows") or metrics.get("card"):
        return "products"
    if metrics.get("absence_markers"):
        return "absence_then_asked" if metrics.get("asks_back") else "asserted_absence"
    if metrics.get("asks_back"):
        return "asked_back"
    return "other"
...
def summarise(records: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    for key, items in sorted(by.items()):
        ...
        summary[key] = {
            ...
            "outcomes": {c: sum(1 for i in items if i.get("outcome") == c)
                         for c in sorted({str(i.get("outcome")) for i in items})},
            "searched_turns": sum(1 for i in items if "search_products" in str(i.get("tools_called") or "")),
        }
    return summary
...
def main() -> int:
    ...
    try:
        ...
        stores = {
            ...
            "C": seed_store(engine, "C", {"assistant_name": "ريم", "default_language": "arabic",
                                          "reply_tone": "friendly"}, CATALOGUE_C),
        }
