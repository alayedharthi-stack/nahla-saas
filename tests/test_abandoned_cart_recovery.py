...
Architectural contract under test (commit 7f08d9b0 — template-only autopilot):

    Stage 1 — 30 minutes,    template,           NO coupon, first reminder
    Stage 2 — 6  hours,      template,           NO coupon, follow-up reminder
    Stage 3 — 23h45m,        template + coupon,  auto-coupon ON, fires before
...
def test_stage_three_fires_before_meta_24h_window_closes() -> None:
    """The coupon stage must stay inside Meta's 24h service window."""
    steps = _cart_seed()["config"]["steps"]
...
def test_stages_two_and_three_emitted_after_24_hours() -> None:
    """A cart abandoned ~24 h ago should emit stage 2 + stage 3 on the
    same sweep (template-only 3-stage flow)."""
    db, engine = _make_db()
    try:
        tenant = _seed_tenant(db)
        customer = _seed_customer(db, tenant.id)
...
def test_stage_three_not_emitted_before_coupon_delay() -> None:
    """Stage 3 (coupon) must not fire until its delay elapses."""
    db, engine = _make_db()
    try:
        tenant = _seed_tenant(db)
        customer = _seed_customer(db, tenant.id)
        ...
        assert (followups[0].payload or {}).get("step_idx") == 1
    finally:
        db.close(); engine.dispose()
