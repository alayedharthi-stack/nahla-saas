...
def test_clause_5_covers_both_cases_and_names_no_phrase():
    ...
    prevails does it ask. When a search matches nothing, a known kind of
    product is reported as not available, while a name or phrase that is not
    recognisably a kind of product is met with a question — a product or
    something else — not with "the store does not have it". It quotes no
    customer phrase — no word list,
    ...
    assert "نوعًا معروفًا من المنتجات فأخبره بأنه غير متوفر" in clause
    assert "لا يتضح أنها نوع من المنتجات فلا تقل إن المتجر لا يملكه" in clause
    assert "هل يقصد منتجًا أو شيئًا آخر" in clause
    for quote in ("«", "»", '"'):
